...
1. **List tasks**: `rdm task list --project graphql-server` to see open and in-progress tasks.
2. **Show details**: if a task slug was provided in `$ARGUMENTS`, run `rdm task show <slug> --project graphql-server`. Otherwise, present the task list and ask the user which task to work on.
3. **Mark in-progress**: `rdm task update <slug> --status in-progress --no-edit --project graphql-server`
